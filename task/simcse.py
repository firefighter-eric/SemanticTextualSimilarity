...
from model.base_model import SentVector, Similarity
...
class SimCSE(LightningModule):
    def __init__(self, config, from_pretrained=False):
        ...
        self.model = SentVector(self.config, from_pretrained)
        if self.config.mlm_flag:
            self.mlm = BertLMPredictionHead(self.config)
    ...
    def configure_optimizers(self):
        ...
        warm_up_epochs = self.config.warm_up_epochs
